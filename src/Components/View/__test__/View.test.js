import { fireEvent , render, screen } from '@testing-library/react';
import View from '../View';
import '@testing-library/jest-dom/extend-expect';
import { createStore } from "redux";
import { isView } from '../../../Reducers/isView';
import { Provider } from "react-redux";
const store = createStore(isView);
let product = {
            id: "001",
            image: "data:image/jpeg;base64,",
            name: "shirt",
            price: "500",
            quantity: "222"

}
describe('View', () => {
    test('view Details', () => {
      render( <Provider store={store}>
       <View product={product}></View>
      </Provider>
      );
      let productTitle = screen.getByTestId('productTitle')
      expect(productTitle.innerHTML).toEqual("PRODUCT")
    });

    test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let productId = screen.getByTestId('productId')
        expect(productId.innerHTML).toEqual("PRODUCT ID : 001")
      });

      test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let productName = screen.getByTestId('productName')
        expect(productName.innerHTML).toEqual("NAME : shirt")
      });


      test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let productQuantity = screen.getByTestId('productQuantity')
        expect(productQuantity.innerHTML).toEqual("QUANTITY : 222")
      });

      test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let productPrice = screen.getByTestId('productPrice')
        expect(productPrice.innerHTML).toEqual("PRICE : Rs.500")
      });


      test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let productImage = screen.getByTestId('productImage')
        expect(productImage.src).toContain("image/jpeg;base64,");
      });

      test('view Details', () => {
        render( <Provider store={store}>
         <View product={product}></View>
        </Provider>
        );
        let clsBtn = screen.getByTestId('clsBtn')
        fireEvent.click(clsBtn)
      });


  });

